...
<?php
    $html_title = array('zh-tw'=>'進貨明細', 'en'=>'Purchase Details');
...
<body>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    $sql = "SELECT io.dt, c1.catid main, c2.catid sub, c1.name parent, c2.name, o.item, o.size, o.color, o.unit, o.com, io.num FROM Category c1, Category c2, Object o, objectAndCat oc, Inventory i, inAndOut io WHERE c2.catid = oc.catid AND o.oid = i.oid AND o.oid = oc.oid AND c1.catid = c2.parent AND o.oid = io.oid AND io.aid = $purchase AND io.sid = $sem";
                                    ...
                                    foreach ($result as $r) {
                                        ...
                                        echo "<tr class='td-".$r['sub']."'>
                                                <td data-title='".$pdt[$lang]."' class='datetime'>".$r['dt']."</td>
                                                ...
                                                <td data-title='".$com[$lang]."'>".$r['com']."</td>
                                                <td data-title='".$num[$lang]."'>".$r['num']."</td>
                                            </tr>";
                                    }
                                ?>
                            </tbody>
    ...
    <script type="text/javascript">
            ...
            function displayDates(from, to) {
                if(from != '' && to != '') {
                    from = new Date((from.split('/')[0])+'-'+(from.split('/')[1])+'-'+(from.split('/')[2]));
                    to = new Date((to.split('/')[0])+'-'+(to.split('/')[1])+'-'+(to.split('/')[2]));
                    if (from <= to) {
                        var tmp;
                        var arr = $('.datetime');
